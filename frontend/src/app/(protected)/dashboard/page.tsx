...
import { JobTypeChip, BrandSwatch, TagChip } from "@/components/ui/job-type-chip";
    ...
    <RoleGate allowedRoles={["owner", "office_staff", "technician", "dealer"]}>
      <section style={{ padding: "24px", maxWidth: "1400px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "16px", marginBottom: "20px" }}>
          <KpiCard title="Total jobs (7d)" value={metrics ? String(metrics.totalJobs) : "—"} accent="#0A0A0A" />
          <KpiCard title="Completion rate" value={metrics ? `${metrics.resolvedOrCompleted}%` : "—"} accent="#10B981" />
          <KpiCard title="Revisit pending" value={metrics ? String(metrics.revisitPending) : "—"} accent="#F59E0B" />
          <KpiCard title="Cancelled" value={metrics ? String(metrics.cancelled) : "—"} accent="#6366F1" />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr)", gap: "20px" }}>
          <div style={{ backgroundColor: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: "12px", padding: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
              ...
              <h2 style={{ fontSize: "14px", fontWeight: 500, color: "#171717", margin: 0 }}>Needs revisit</h2>
            </div>

            {needsRevisitJobs.length === 0 ? (
              <p style={{ margin: 0, color: "#737373", fontSize: "13px" }}>No jobs currently need revisiting.</p>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid #FDE68A" }}>
                      {['Job ID', 'Customer', 'Technician', 'Waiting', 'Type'].map((heading) => (
                        <th key={heading} style={{ padding: "10px 12px", textAlign: "left", fontSize: "12px", color: "#92400E", fontWeight: 500 }}>
                          {heading}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {needsRevisitJobs.map((job) => (
                      <tr key={job.id} style={{ borderBottom: "1px solid #FEF3C7", cursor: "pointer" }} onClick={() => router.push(`/jobs/${job.id}`)}>
          ...
          <div style={{ backgroundColor: "#fff", borderRadius: "12px", border: "1px solid #E5E5E5", overflow: "hidden" }}>
            <div style={{ padding: "14px 16px", borderBottom: "1px solid #E5E5E5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h2 style={{ fontSize: "14px", fontWeight: 500, color: "#171717", margin: 0 }}>Recent jobs</h2>
            ...
            {recentJobs.length === 0 ? (
              <div style={{ padding: "40px", textAlign: "center", color: "#737373" }}>
                <Briefcase size={32} strokeWidth={1} style={{ margin: "0 auto 12px", display: "block", opacity: 0.4, color: "#D4D4D4" }} />
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid #E5E5E5" }}>
                      {['Job ID', 'Status', 'Type', 'Brand', 'Customer', 'Assigned to'].map((heading) => (
                        <th key={heading} style={{ padding: "10px 16px", textAlign: "left", fontSize: "12px", color: "#737373", fontWeight: 500 }}>
                          {heading}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {recentJobs.map((job) => (
                      <tr key={job.id} onClick={() => router.push(`/jobs/${job.id}`)} style={{ borderBottom: "1px solid #F5F5F5", cursor: "pointer" }}>
