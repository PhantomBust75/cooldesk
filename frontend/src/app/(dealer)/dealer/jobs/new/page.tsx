          ...
          {value < total ? (
            <div
              style={{
                flex: 1,
                height: "2px",
                backgroundColor: value < current ? "#10B981" : "#E5E5E5",
                minWidth: "40px",
              }}
            />
          ) : null}
...
export default function DealerLogNewJobPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createDealerJob({
        ...
        ...(form.type === "installation" && form.notes.trim()
          ? { installationNotes: form.notes.trim() }
          : {}),
        ...(form.type === "complaint" && form.notes.trim()
          ? { issueDescription: form.notes.trim() }
          : {}),
      }),
    onSuccess: (data) => {
    ...
    onError: (error) => {
      const msg =
        error instanceof ApiError ? error.message : "Unable to create job.";
      setErrorMessage(msg);
  ...
  const step2Valid = Boolean(
    form.customerName.trim() && form.phone.trim() && form.address.trim(),
  );
  ...
  const notesLabel =
    form.type === "complaint" ? "Issue Description" : "Installation Notes";

  if (createdJobId) {
    return (
      <section
        style={{ padding: isMobile ? "16px" : "24px", maxWidth: "720px" }}
      >
        <div
          style={{
  ...
  return (
    <section style={{ padding: isMobile ? "16px" : "24px", maxWidth: "720px" }}>
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (step === 3 && step3Valid) createMutation.mutate();
        }}
        style={{
          backgroundColor: "#fff",
          borderRadius: "12px",
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        color: form.type === type ? "#0A0A0A" : "#525252",
              ...
              <div>
                ...
                <select
                  ...
                  style={{
                    ...
                    fontSize: "13px",
                    outline: "none",
                    color: form.brandId ? "#171717" : "#A3A3A3",
                  }}
                >
