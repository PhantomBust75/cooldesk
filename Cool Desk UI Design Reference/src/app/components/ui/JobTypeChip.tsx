import type { JobType, JobSource } from '../../data/mockData';
import { Wrench, MessageCircleWarning, ArrowRight, Building2 } from 'lucide-react';

interface TypeChipProps { type: JobType }
interface SourceChipProps { source: JobSource; dealerName?: string }

/** Plain text + icon — NO background fill, no pill. */
...
/** Full-pill tinted tag chip. Chronic=rose, Frequent=ochre, Repeat=slate, generic=gray. */
export function TagChip({ label, variant }: { label: string; variant?: 'chronic' | 'frequent' | 'repeat' | 'gray' }) {
  const themes = {
...
/** Brand name text — no dot. */
export function BrandSwatch({ name, colorHex: _colorHex }: { name: string; colorHex: string | null }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: '13px', color: '#64748B' }}>
      {name}
    </span>
  );
}